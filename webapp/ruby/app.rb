...
class App < Sinatra::Base
  ...
  get '/message' do
    ...
    statement = db.prepare('SELECT * FROM message JOIN user ON message.user_id = user.id WHERE id > ? AND channel_id = ? ORDER BY id DESC LIMIT 100')
  ...
  get '/history/:channel_id' do
    ...
    n = 20
    statement = db.prepare('SELECT * FROM message JOIN user ON message.user_id = user.id WHERE channel_id = ? ORDER BY id DESC LIMIT ? OFFSET ?')
